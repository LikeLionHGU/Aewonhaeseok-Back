...
| B6 기준치 | 기준 세트 · 기준치 조회 · 초과 판정 |
| B7 분석 | 조건 기반 통계 · 실행 이력 · 근거 상세 |
| B1 계정 | **두지 않기로 결정** (2026-08-12) |

## 남은 일

**자연어 파싱 하나뿐이다.** `POST /analyses/parse` — 문장을 분석 조건으로 해석하는 부분.
없어도 조건 선택 방식으로 결과가 나오므로 데모는 성립한다.

**기준치 값 확인.** `src/main/resources/standards/standard_limits.csv`가 전부 `source=demo`다.
별표 13 원문과 대조해 옮겨 넣고 `law`로 바꿔야 실제 판정 근거로 쓸 수 있다.

## 인증

**로그인을 두지 않기로 했다(2026-08-12).** 모든 엔드포인트가 인증 없이 열려 있고
기관별 데이터 분리도 하지 않는다.

다만 용어 판정에는 판정자 이름을 받는다(`reviewed_by`). 사전 담당자가 나중에
"이 동의어를 누가 넣었나"를 확인해야 하고, `apply_review()`가 그 값을 사전 이력에
기록하기 때문이다. 비워 두면 `미상`으로 저장된다.
