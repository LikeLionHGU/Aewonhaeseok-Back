...
# WQ-074(수위)·WQ-075(유량) 폐기 — 2026-08-10.
# 수질오염물질이 아니라 수문 관측값이라 물환경보전법 별표 13에 근거가 없었다.
# 타깃이 폐수측정 업체로 확정되면서 측정 대상 밖이 되어 사전에서 제외한다.
...
# --- 초안(AI 생성) 동의어 폐기 — 2026-08-10 -------------------------------
# 초안 xlsx에서 AI가 추정으로 채운 표기 61건이 여기 있었다. 두 번의 실측 결과
# 매핑 기여가 사실상 없어서 전량 폐기한다.
#   · 샘플 6개 파일: 61건을 빼고 재실행해도 170컬럼 판정이 1건도 바뀌지 않음
#   · 홀드아웃(충남 69컬럼): 자동확정 61건 중 AI 표기에 의존한 것은 1건뿐이었고,
#     그 1건('수온')조차 실데이터에 실재하는 표기라 아래에서 관측 근거로 승격했다
# 근거 없는 추정 표기를 사전에 두면 "미검증 20%"라는 약점만 남는다.
# 새 표기는 실데이터가 들어올 때 observed 근거와 함께 쌓는다.

# 위 61건 중 실데이터 컬럼명에 그대로 존재함이 확인된 4건 — 관측 근거로 승격
for _code, _sfc in [
    ("WQ-001", "생물학적 산소요구량"),
    ("WQ-002", "총유기탄소"),
    ("WQ-004", "부유물질"),
    ...
    s(_code, _sfc, "DATAGO_ANYANG", stype="column_name", review="verified",
      snippet="컬럼명 그대로 관측",
      note="초안 AI 표기로 등록됐으나 실데이터 컬럼명에 실재함을 확인해 관측 근거로 승격(2026-08-10)")
