...
d("MD-002", "측정소코드", "Station Code", "MSRSTN_CD", "코드", "code", "station", "",
  ...
  "공통표준용어 미등재(exact match 0건). 공통표준단어 측정소(MSRSTN)+코드(CD) 조합으로 만든 "
  "기관표준용어이며 공식 표준용어가 아니다. 근접 표준용어는 '측정지점번호' MSPT_NO(번호V50)",
...
d("MD-004", "측정시설명", "Facility Name", "-", "", "text", "station", "",
  "정수장·물재생센터 등 시설 단위 관측의 지점명", "",
  basis=UNCHECKED, review="draft")
d("MD-005", "측정시설번호", "Facility Number", "-", "", "code", "station", "", "", "",
  basis=UNCHECKED, review="draft")
d("MD-030", "채수지점번호", "Sampling Spot Number", "MSPT_NO", "번호V50", "code",
  "station", "", "", "근접 표준용어 '측정지점번호'(MSPT_NO)를 차용. 원 표기와 다르므로 확인 필요",
  basis="공통표준용어 '측정지점번호' 차용", review="draft")
...
d("MD-008", "측정연도", "Measurement Year", "-", "", "integer", "time", "YYYY",
  "연/월을 분리 저장하는 소스가 많다. 측정일자로 통합 가능하면 통합", "",
  basis=UNCHECKED, review="draft")
d("MD-009", "측정월", "Measurement Month", "-", "", "integer", "time", "MM",
  "'02'처럼 zero-pad 문자열로 오는 소스 있음", "", basis=UNCHECKED, review="draft")
d("MD-010", "측정회차", "Measurement Round", "-", "", "text", "time", "",
  "'1회차' 같은 문자열. 분기/회차 개념이 소스마다 다름", "",
  basis=UNCHECKED, review="draft")
d("MD-011", "시료채취일자", "Sampling Date", "SPCM_PCKNG_YMD", "연월일C8", "date", "time",
  ...
  "어느 쪽을 쓸지 팀 규칙이 필요하다")
d("MD-012", "측정시각", "Measurement Time", "-", "", "text", "time", "HH24:MI", "", "",
  basis=UNCHECKED, review="draft")
...
d("MD-020", "읍면동명", "Town Name", "-", "", "text", "space", "", "", "",
  basis=UNCHECKED, review="draft")
d("MD-021", "리명", "Village Name", "-", "", "text", "space", "", "", "",
  basis=UNCHECKED, review="draft")
...
d("MD-023", "용수구분명", "Water Use Type", "-", "", "text", "sampling", "",
  "생활정수/공업용수 등", "", basis=UNCHECKED, review="draft")
d("MD-024", "채수수심", "Sampling Depth", "-", "", "numeric", "sampling", "m",
  "국립환경과학원 wmdep. 같은 지점 다른 수심의 값이 별도 행으로 오는 경우 주의", "",
  basis=UNCHECKED, review="draft")
...
d("MD-027", "행번호", "Row Number", "-", "", "integer", "record", "",
  "API 응답 전용. 데이터 자체의 의미는 없다", "", basis="소스 관행", review="draft")
d("MD-028", "전체결과수", "Total Count", "-", "", "integer", "record", "",
  "API 페이징 메타", "", basis="소스 관행", review="draft")
...
# --- 인천 CSV (wide 피벗) ---------------------------------------------------
for code, sfc in [("WQ-009", "시천천_심곡천수온"), ("WQ-008", "시천천_심곡천수소이온농도"),
                  ("WQ-060", "시천천_심곡천용존산소"),
                  ("WQ-001", "시천천_심곡천생물학적 산소요구량")]:
    s(code, sfc, "DATAGO_INCHEON", review="draft",
      snippet="구분,시천천_심곡천수온,시천천_심곡천수소이온농도,...",
      note="하천명이 컬럼명에 병합된 wide 구조. 접두 '<하천명>_'을 떼야 항목이 드러난다 — "
           "동의어 등록보다 파싱 규칙으로 처리할 것")
