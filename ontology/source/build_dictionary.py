...
SOURCES = [
    ...
    ("LAW_FORM18", "물환경보전법 시행규칙 별지 제18호서식 폐수배출시설 및 수질오염방지시설 운영일지",
     "환경부", "legal", "https://www.law.go.kr/LSW/lsBylInfoPLinkR.do?bylCls=BF&bylNo=0018",
     "개정 2024. 11. 7. PDF 2쪽 텍스트 추출본 전수 대조. 추출문은 표의 행·열 경계가 소실되고 "
     "자간 공백이 삽입된다('전 일  지 침', '약 품  소 모 량', '유 량 계', 'n- H ex'). "
     "칸 위치·띄어쓰기 확정은 원 서식 이미지 재확인 필요", "UTF-8", "legal"),

    ("LAW_FORM20", "물환경보전법 시행규칙 별지 제20호서식 폐수배출시설 운영일지",
     "환경부", "legal", "https://www.law.go.kr/LSW/lsBylInfoPLinkR.do?bylCls=BF&bylNo=0020",
     "개정 2018. 1. 17. 시행령 제33조제2호·제3호에 따라 폐수를 처리하는 사업자가 적는다. "
     "총 10칸. 수량 단위가 제18호서식의 ㎥가 아니라 L", "UTF-8", "legal"),

    ("TMS_NOTICE", "수질원격감시체계 관제센터 운영 등에 관한 규정(환경부고시)",
     "환경부", "legal", "https://www.law.go.kr/LSW/admRulLsInfoP.do?admRulSeq=2100000265818",
     "국가법령정보센터 본문 중 핵심 조문 발췌본으로 대조 — 전문이 아니다. "
     "별표1(비정상자동측정자료 선별기준)·별표2(대체자료 생성방법)·별표3(24시간 평균치 생성방법) "
     "미포함. 조문 단위 표기는 관보 원문 재확인 필요", "UTF-8", "legal"),
...
TMS_REG = "수질원격감시체계 관제센터 운영 등에 관한 규정(환경부고시)"
FORM18 = ("물환경보전법 시행규칙 별지 제18호서식 "
          "폐수배출시설 및 수질오염방지시설 운영일지(개정 2024. 11. 7.)")
FORM20 = ("물환경보전법 시행규칙 별지 제20호서식 "
          "폐수배출시설 운영일지(개정 2018. 1. 17.)")
...
# WQ-074(수위) 폐기 — 2026-08-10.
# 하천 수문 관측값이라 별표 13에 근거가 없고, TMS 의무 전송 항목에도 없다.
# 폐수측정 타깃에서는 측정 대상 밖이므로 사전에서 제외한다.

# 유량은 폐수 도메인에서 법정 측정 항목이다. 별표 13의 '수질오염물질'은 아니지만
# 물환경보전법 제38조의2에 따른 측정기기 의무 부착 항목이고, TMS가 관제센터로
# 전송받는 6개 항목(pH·TOC·SS·T-N·T-P·유량) 중 하나다. 배출부과금 산정의
# '초과량' 계산이 농도×유량이라 유량 없이는 부과금 자체가 산출되지 않는다.
m("WQ-075", "유량", "Flow Rate", "-", "㎥/s",
  "수문", "물환경보전법 제38조의2 및 " + TMS_REG, "", "non_statutory", False, "verified",
  "별표 13의 수질오염물질은 아니나 폐수 TMS 의무 전송 6항목 중 하나. "
  "별지 제18호서식 11번 '수질자동측정기기 등의 측정항목별 점검내용'에 '유량계'로 등장한다. "
  "2026-08-10 하천 기준으로 근거 없음 판정해 삭제했다가 폐수 원전 확인 후 복구")
...
# ---------------------------------------------------------------------------
# 폐수 도메인 (2026-08-10 추가) — 타깃이 폐수측정 업체로 확정되면서
# 하천·약수터 기준 사전이 못 덮는 축을 서식 원문으로 채운다.
# 분류 규칙: 원전이 "재거나 계량해 칸에 적는 물리량"으로 세운 것만 측정항목(WQ).
#            그 값을 식별·구분·판정·집계하는 것은 메타(MD).
# 이번 배치는 근거가 가장 강한 필수 항목만 넣는다 — 별지 제18호서식 말미가
# "제1호부터 제4호까지는 … 반드시 적어야 하며"로 기재 의무를 구분한다.
# ---------------------------------------------------------------------------
m("WQ-077", "폐수발생량", "Wastewater Generation", "-", "㎥/일",
  "수량", FORM18 + " 제3호 우측 표 항목 행 '폐수발생량'", "", "non_statutory", False, "verified",
  "제18호서식·제20호서식 양쪽에 완전히 같은 표기로 나오는 유일한 수량 항목이라 근거가 가장 강하다. "
  "단위가 갈린다 — 제18호 표두 '배 출 량  및 사 용 량(㎥/일)', 제20호 '폐수발생량(L)'. "
  "1000배 차이라 적재 전 정규화 필수(엔진 규칙). "
  "제20호서식을 '소규모용'이라 부르지 말 것 — 원문은 '시행령 제33조제2호 및 제3호에 따라 "
  "폐수를 처리하는 사업자'라고만 한다")
m("WQ-078", "폐수배출량", "Wastewater Discharge", "-", "㎥/일",
  "수량", FORM18 + " 제3호 표제 '용수 공급원별 사용량과 폐수배출량' 및 우측 표 항목 행 '폐수배출량'",
  "", "non_statutory", False, "verified",
  "발생량(공정에서 생긴 양)과 배출량(방류구로 나간 양)은 서식이 칸을 따로 둔 별개 값이다. "
  "제20호서식에는 없다. WQ-075 유량(㎥/s, 순시)과 차원이 다르므로 합치지 말 것. "
  "'배출부과금 산정의 기초 수량'이라는 설명은 TMS 고시 제15조에 없다 — 인용 금지")
m("WQ-079", "냉각수량", "Cooling Water Volume", "-", "㎥/일",
  "수량", FORM18 + " 제3호 우측 표 항목 행 '냉각수량'", "", "non_statutory", False, "verified",
  "폐수와 구분해 따로 집계하도록 서식이 칸을 나눠 놓았다. 폐수배출량으로 오분류 주의")
m("WQ-080", "소모(증발량)", "Consumption (Evaporation)", "-", "㎥/일",
  "수량", FORM18 + " 제3호 우측 표 항목 행 '소모(증발량)'", "", "non_statutory", False, "verified",
  "괄호까지 포함한 표기가 서식 원문 그대로다. 컬럼명 정규화 시 괄호 처리 규칙 필요. "
  "물질수지 등식(사용량 = 발생 + 냉각 + 소모 + 재사용 + 생활용수)은 서식에 없는 추론이므로 "
  "근거가 아니라 엔진 검증식으로 둔다")
m("WQ-081", "재사용량", "Reuse Volume", "-", "㎥/일",
  "수량", FORM18 + " 제3호 우측 표 항목 행 '재사용량'", "", "non_statutory", False, "verified", "")
m("WQ-082", "생활용수량", "Domestic Water Volume", "-", "㎥/일",
  "수량", FORM18 + " 제3호 우측 표 항목 행 '생활용수량'", "", "non_statutory", False, "verified",
  "MD-023 용수구분명과 이름이 비슷하나 전혀 다르다 — 이쪽은 수량 값, 저쪽은 구분 코드")
m("WQ-083", "용수사용량", "Water Intake", "-", "㎥/일",
  "수량", FORM20 + " '용수사용량(L)'", "", "non_statutory", False, "verified",
  "'용수사용량' 연속 표기는 제20호서식에만 있다. 제18호서식은 표제 '용수 공급원별 사용량'과 "
  "좌측 표 열 '사용량(㎥/일)'로 표기가 갈리므로 그쪽은 동의어 근거로만 쓴다. "
  "단위는 제18호(㎥/일)를 표준으로 잡고 제20호(L)를 엔진에서 환산")
m("WQ-084", "슬러지 발생량", "Sludge Generation", "-", "㎥",
  "수량", FORM18 + " 제4호 표제 '슬러지의 발생량 및 처리량' 및 표 '슬러지발생량(㎥)'",
  "", "non_statutory", False, "verified",
  "PDF 추출문은 행 라벨과 열이 붙어 '슬러지발생량(㎥)'로 나온다. 표제어는 제3호 폐수발생량과의 "
  "구분을 위해 '슬러지 발생량'으로 띄우되, 원전 열 표기가 '발생량(㎥)'임을 여기 남긴다")
m("WQ-085", "슬러지 처리량", "Sludge Treatment", "-", "㎥",
  "수량", FORM18 + " 제4호 표 열 '처리량(㎥)'", "", "non_statutory", False, "verified",
  "표제어는 표제+열 조합. '보관량 = 전일보관 + 발생 - 처리' 검증식은 서식 원문에 없는 추론이므로 "
  "근거가 아니라 엔진 규칙으로 둔다(서식에 전일/금일 칸도 없다)")
m("WQ-086", "슬러지 보관량", "Sludge Storage", "-", "㎥",
  "수량", FORM18 + " 제4호 표 열 '보관량(㎥)'", "", "non_statutory", False, "verified",
  "표제어는 표제+열 조합. 단 '보관량'은 표제('슬러지의 발생량 및 처리량')에 포함되지 않아 "
  "발생량·처리량보다 맥락 근거가 약하다 — 근거는 표 소속뿐이다")
m("WQ-087", "함수율", "Water Content", "-", "%",
  "운전지표", FORM18 + " 제4호 표 '함수율(%)'", "", "non_statutory", False, "verified",
  "단위 %가 서식에 명시된 드문 칸. 서식 각주가 기록 주기까지 규정한다 — "
  "'※ 함수율(%)란은 슬러지를 처리하는 경우 또는 슬러지를 적정하게 관리ㆍ처리할 수 있는 "
  "주기(월 1회 등)마다 작성합니다.' 즉 다른 일일 항목과 기록 주기가 다르다. "
  "측정 대상이 물이 아니라 슬러지이므로 시료 종류를 구분하지 않으면 방류수 항목과 뒤섞인다. "
  "'건조중량 환산에 필수'는 원전에 없는 도메인 지식")
...
d("MD-030", "채수지점번호", "Sampling Spot Number", "-", "", "code",
  ...
  "채수 장소와 측정 장소가 같은 실체인지도 원본 정의서 없이는 확인되지 않는다. 2026-08-10 폐수 원전 대조 결과 미해결 확정 — '채수지점' 0건. 폐수에서 이 자리를 차지하는 실체는 '방류구'이나 개념이 달라 재정의가 아니라 신규 등재로 처리했다(MD-033). 이 코드에는 경기 약수터 SPOT_NO가 바인딩돼 있어 개명하면 약수터 채수지점이 폐수 방류구에 묶인다. 적용범위를 하천·약수터로 한정한다",
...
d("MD-010", "측정회차", "Measurement Round", "-", "", "text", "time", "",
  ...
  "모르면 확정할 수 없다. 표준은 회차→차수 매핑을 제공하지 않는다(이음동의어 0건). 2026-08-10 폐수 원전 대조 결과 미해결 확정 — 별지 제18·20호서식과 수질원격감시체계 관제센터 운영 등에 관한 규정에 '회차·차수·횟수·순번' 0건. 제18호서식 제1·2호의 1~24는 하루의 시간대이고 제3호의 1호·2호는 계량기 호수다. TMS의 5분 자료·시간 자료·24시간 평균치는 집계 주기이지 순번이 아니다. 적용범위를 하천·먹는물 정기검사로 한정한다",
...
d("MD-024", "채수수심", "Sampling Depth", "-", "", "numeric", "sampling", "",
  ...
  "일'이라 쓴다. 채수 WTSMP는 확정이나 꼬리 단어를 정해야 조합이 성립한다. 2026-08-10 폐수 원전 대조 결과 미해결 확정 — '수심·깊이·채수' 각 0건. 나아가 폐수는 수심 방향으로 층이 진 수체가 아니라 방류구와 조에서 채취한다(제18호서식 제11호가 시료 경로를 '시료채취조 → 시료도입[튜빙(tubing: 배관)류]'로 규정). 폐수 도메인 적용 제외",
...
# MD-026 재정의 — 구 표제어 '초과항목명'은 두 폐수 원전에 0건인 창작 표기였다.
d("MD-026", "초과오염물질", "Exceeded Pollutant", "-", "", "text", "quality", "",
  "기준을 초과한 오염물질을 담는다 — 측정항목 사전(WQ-*)과 대조해 파싱 가능. "
  "경기 EXCESS_ITEM_NM의 실제 응답값은 여전히 미확보라 한 건만 담는지 구분자로 여러 건을 "
  "묶는지 모른다(이 미결은 표제어 교체로 해소되지 않는다 — 고시는 관제센터 저장 구조를 "
  "규정할 뿐 경기 API의 값 카디널리티를 정해 주지 않는다)",
  "2026-08-10 표제어 교체. 구 표제어 '초과항목명'은 두 폐수 원전에 0건인 창작 표기였다. "
  "근거: 수질원격감시체계 관제센터 운영 등에 관한 규정 제5조제3호나목 — "
  "'배출허용기준등을 초과한 자료의 사업장별, 방류구별 초과오염물질·초과농도·초과시간 및 "
  "초과량 저장'. 같은 목이 초과 사실을 4개 필드로 분해하므로 이 용어는 단독이 아니라 "
  "MD-052 초과농도·MD-046 초과시간·MD-053 초과량과 한 세트로 쓴다. "
  "적용범위 주의 — 경기 약수터 EXCESS_ITEM_NM에 이 용어를 붙일 때 초과 기준은 "
  "배출허용기준이 아니라 먹는물 수질기준이며, 초과 대상이 '오염물질'이 아닌 항목(탁도 등)일 "
  "수 있다. 행정안전부 공통표준용어에는 여전히 미등재(exact 0건, 8차 20251101)",
  basis="수질원격감시체계 관제센터 운영 등에 관한 규정 제5조제3호나목", review="verified")
...
d("MD-032", "미검사사유", "Non-Inspection Reason", "-", "", "text", "quality", "",
  ...
  "근접 용어 '미사용사유' UNSD_RSN(내용V4000)은 개념이 다르다. 표준 미비로 해소 불가. 2026-08-10 폐수 원전 대조 결과 미해결 확정 — '미검사' 0건, 별지 서식에는 '사유·검사' 자체가 0건. TMS 고시의 '개선사유'(제5조제9호·제9조제5항)와 제12조제3항 무효사유는 모두 '기기는 돌았으나 값을 믿을 수 없는' 경우이지 '검사를 하지 않은' 경우가 아니다. 합치면 무효자료와 미실시가 섞인다",
  basis="공통표준용어·표준단어 미등재 — 소스 관행", review="draft")
# MD-027(행번호)·MD-028(전체결과수) 폐기 — 2026-08-10.
# 둘 다 Open API 응답의 페이징 봉투 필드(rowno / totalCount)이지 수질 도메인 용어가 아니다.
# 행안부 공통표준용어에 없는 것이 당연하고, 어느 도메인 원전에서도 근거가 나올 수 없다.
# 관측 데이터의 의미를 담지 않으므로 표준 사전의 대상이 아니라고 판단해 제외한다.
...
# --- 폐수 도메인 메타 (2026-08-10 추가, 필수 17건) -------------------------
# 근거: 별지 제18·20호서식 + 수질원격감시체계 관제센터 운영 등에 관한 규정
d("MD-033", "방류구", "Discharge Outlet", "-", "", "text", "station", "",
  "폐수 자료의 지점 그레인 그 자체. 한 사업장에 여럿 존재하고 방류구마다 측정항목과 "
  "배출허용기준이 따로 설정된다. 이 축이 없으면 방류구별 배출현황·기준·초과자료가 "
  "사업장 단위로 뭉개져 덮어쓰기가 난다. value_type을 code가 아니라 text로 둔다 — "
  "'방류구번호'·'방류구코드' 표기는 두 원전 grep 0건이라 식별자 형식을 정할 근거가 없다",
  "근거 5곳: 제5조제2호가목 '사업장별, 방류구별, 오염물질별 배출현황' / 제3호나목 "
  "'사업장별, 방류구별 초과오염물질…' / 제4호나목 '방류구 관리에 필요한 기초사항' / "
  "다목 '방류구별 측정항목' / 마목 '방류구별, 오염물질별 배출허용기준등 설정 및 관리'. "
  "별지 제18호서식에는 '방류구'가 0건이고 제10호의 '방류수'는 시료의 종류이지 지점 "
  "식별자가 아니므로 근거로 쓰면 안 된다. MD-030 채수지점번호와는 별개 실체다",
  basis=TMS_REG + " 제5조제4호나목·다목·마목", review="verified")
d("MD-034", "사업장 명칭", "Business Site Name", "-", "", "text", "station", "",
  "폐수 데이터의 최상위 집계 단위. 고시가 배출현황·부과금 산정·초과자료·공개자료를 모두 "
  "'사업장별'로 규정한다. 사업장 식별 '코드/번호'에 해당하는 표현은 두 원전 어디에도 없어 "
  "코드 용어는 만들지 않았다",
  "제13조의2제1항 '사업장별 명칭 및 소재지, 시설용량 등 일반 현황'. 원전 표기가 "
  "'사업장별 명칭'이므로 표제어를 '사업장 명칭'으로 두었다('사업장' 단독은 개체명이지 "
  "컬럼명이 아니다). 같은 축의 다른 표기 병존 — 제5조제10호 '측정기기부착사업장', "
  "제5조제9호 '측정기기부착사업자등'(사업자). 별도 행으로 세우지 않고 여기 기록한다",
  basis=TMS_REG + " 제13조의2제1항", review="verified")
d("MD-035", "처리방법", "Treatment Method", "-", "", "text", "station", "",
  "방지시설의 처리 공법. 값 도메인이 서식에 열거돼 있지 않으므로 자유 텍스트로 둔다",
  "별지 제18호서식 제2호 '수질오염방지시설 가동시간대(처리방법:                           )' "
  "표제 괄호 안 기입란. 제8호가 '(생물화학적 처리시설의 경우)'로 한정되므로 이 값이 "
  "어떤 운전지표 칸이 채워져야 정상인지를 결정한다 — 조건부 기재 규칙의 입력. "
  "facet은 시료 채취 맥락이 아니라 시설 속성이므로 sampling이 아닌 station",
  basis=FORM18 + " 제2호", review="verified")
d("MD-039", "폐수배출시설 가동(조업)시간대", "Discharge Facility Operating Hours",
  "-", "", "text", "time", "",
  "1시간 단위 24칸 on/off 격자다. 스칼라 시각값인 MD-012 측정시각으로는 표현할 수 없어 "
  "24비트 벡터 자료형 정의가 함께 필요하다(사전에 선례 없음)",
  "별지 제18호서식 제1호 표제 '폐수배출시설 가동(조업)시간대' 및 표 머리 '시간대/구분'과 "
  "값 칸 1~24. 서식 말미 주석상 필수 기재 구간(제1~4호). "
  "'※ 시간대 표시는 검은색'은 제1호가 아니라 제2호 격자 뒤에 붙어 있다",
  basis=FORM18 + " 제1호", review="verified")
d("MD-040", "수질오염방지시설 가동시간대", "Prevention Facility Operating Hours",
  "-", "", "text", "time", "",
  "제1호와 별도의 24칸 격자. 두 격자를 하나로 접으면 배출시설과 방지시설의 시간대가 뭉개진다",
  "별지 제18호서식 제2호 표제 '수질오염방지시설 가동시간대(처리방법:  )' 및 값 칸 1~24. "
  "필수 기재 구간. 배출시설은 가동 중인데 방지시설이 정지한 시간대가 위반 신호라는 것은 "
  "원전에 없는 활용 해석이므로 엔진 규칙으로 둔다",
  basis=FORM18 + " 제2호", review="verified")
d("MD-041", "분석일", "Analysis Date", "-", "", "date", "time", "YYYY-MM-DD",
  "시료를 채취한 날과 분석한 날이 다를 수 있다. 주의 — MD-006 측정일자의 동의어에 이미 "
  "'검사일자'가 있어 의미가 상당 부분 겹친다. 자가측정 데이터에서 분석일과 측정일자를 "
  "가르는 규칙을 정하지 않으면 중복 매핑이 난다",
  "별지 제18호서식 제10호 '수질오염물질 측정내용' 항목 열 마지막. "
  "'시료 보관시간 초과 판정에 필요'는 원문 밖 해석",
  basis=FORM18 + " 제10호", review="verified")
d("MD-046", "초과시간", "Exceedance Duration", "-", "", "text", "time", "",
  "초과가 지속된 시간. MD-007 측정일시(시점)와 다른 구간 개념이라 별도 용어가 필요하다. "
  "고시가 지속시간인지 초과 시각인지 정의하지 않아 자료형을 numeric으로 확정하지 않았다",
  "제5조제3호나목이 저장하도록 규정한 초과 4요소 중 하나 — "
  "'초과오염물질·초과농도·초과시간 및 초과량'. MD-026·MD-052·MD-053과 한 세트. "
  "'배출부과금 산정의 직접 입력값(제5조제2호마목·제15조)'이라는 보강 근거는 원전에 없다 — "
  "제5조제2호마목은 '사업장별 배출부과금 부과자료 산정', 제15조제1항은 부과금의 구성만 "
  "규정할 뿐 초과시간과의 연결이 어디에도 없다. 이 문장을 근거로 쓰지 말 것",
  basis=TMS_REG + " 제5조제3호나목", review="verified")
d("MD-050", "측정방법", "Measurement Method", "-", "", "text", "sampling", "",
  "같은 항목이라도 연속자동측정과 수(手)분석은 값의 성격이 다르다. 다만 값 도메인 2종을 "
  "원전이 열거한 것은 아니므로 코드값을 임의로 정하지 말 것",
  "제5조제4호라목 '부착된 측정기기별 특성 및 측정방법'. 보강 — 제2조제2호나목 "
  "상대정확도시험이 '연속자동측정방법 측정값과 항목별 주 시험방법 수(手)분석값 간 오차율 "
  "비교'로 정의돼 두 계열이 구분되는 실체임을 뒷받침한다",
  basis=TMS_REG + " 제5조제4호라목", review="verified")
d("MD-052", "초과농도", "Exceedance Concentration", "-", "", "numeric", "quality", "",
  "초과 4요소 중 하나. 주의 — 원전은 나열만 할 뿐 이 값이 (측정농도 − 기준값)인지 초과 당시의 "
  "측정농도인지 정의하지 않는다. 이 추론을 정의란에 근거처럼 쓰지 말 것. "
  "참고로 '측정농도'는 제5조제5호에 별도로 존재한다",
  "제5조제3호나목 '초과오염물질·초과농도·초과시간 및 초과량'. "
  "물리량이지만 개별 측정 칸이 아니라 고시가 저장 대상으로 규정한 산출값이라 측정항목이 "
  "아니라 메타에 둔다(이 사전의 분류 규칙 예외 조항)",
  basis=TMS_REG + " 제5조제3호나목", review="verified")
d("MD-053", "초과량", "Exceedance Load", "-", "", "numeric", "quality", "",
  "초과 4요소 중 하나. '초과농도 × 유량으로 산출된다'와 '초과배출부과금의 과표다'는 "
  "두 원전 어디에도 없는 창작이므로 근거로 쓰지 말 것 — 제15조제1항은 부과금이 "
  "기본+초과로 구성된다고만 하고 과표를 규정하지 않는다",
  "제5조제3호나목이 저장하도록 규정한 4개 필드 중 하나",
  basis=TMS_REG + " 제5조제3호나목", review="verified")
d("MD-054", "배출허용기준", "Effluent Limit", "-", "", "numeric", "quality", "",
  "폐수 데이터의 판정 기준값. MD-025 측정결과적합여부는 적합/부적합 결과만 담고 무엇에 "
  "견주었는지를 담지 못한다. 고시가 기준값을 방류구별·오염물질별로 '설정 및 관리'하는 "
  "저장 대상으로 명시하므로 기준값 자체가 데이터 항목이다",
  "제5조제4호마목 '방류구별, 오염물질별 배출허용기준등 설정 및 관리', 제13조제3항. "
  "표기 주의 — 제5조 계열은 전부 '배출허용기준등'(등 포함)이고 '등' 없는 표기는 "
  "제13조제3항뿐이다. 표제어는 '등' 없는 쪽을 채택하고 '배출허용기준등'을 동의어로 둔다",
  basis=TMS_REG + " 제5조제4호마목", review="verified")
d("MD-055", "방류수수질기준", "Effluent Quality Standard", "-", "", "numeric", "quality", "",
  "배출허용기준과 병렬로 쓰이는 별개의 법정 기준(공공폐수처리시설 계열). 한 컬럼으로 뭉치면 "
  "어느 기준으로 초과 판정됐는지 추적이 끊긴다. "
  "주의 — MD-033과 별개로 별지 제18호서식 제10호의 값 '방류수'와 문자열이 겹치므로 "
  "사전 검색 시 충돌 방지 필요",
  "제13조제3항 '24시간 평균치가 방류수수질기준 또는 배출허용기준 초과 시 15일 이내 확정 통보'. "
  "tms.md는 발췌본이라 관보 원문의 띄어쓰기('방류수 수질기준' 여부)는 재확인 필요",
  basis=TMS_REG + " 제13조제3항", review="verified")
d("MD-066", "측정대행업소명", "Measurement Agency Name", "-", "", "text", "organization", "",
  "타깃 사용자(폐수측정 업체) 자신이 기재되는 칸이다. 현재 organization 축은 MD-022 기관명 "
  "하나뿐이라 역할이 다른 업체들이 전부 한 칸에 뭉개진다. MD-068 위탁처리업소명(슬러지)·"
  "MD-069 처리업소명(폐수)과 반드시 구분할 것",
  "별지 제18호서식 제10호 아래 '※ 분석을 위탁하는 경우 측정대행업소명:'",
  basis=FORM18 + " 제10호", review="verified")
d("MD-067", "분석자명", "Analyst Name", "-", "", "text", "organization", "",
  "자가분석 시의 책임자. MD-066과 배타적으로 채워지는 짝 칸이라 함께 등재해야 자가/위탁 "
  "두 경로가 구분된다. 개인 성명 필드이므로 공개 데이터셋에서는 비식별 대상일 수 있다",
  "별지 제18호서식 제10호 아래 '※ 사업장에서 분석하는 경우 분석자명:'",
  basis=FORM18 + " 제10호", review="verified")
d("MD-072", "24시간 평균치", "24-Hour Average", "-", "", "numeric", "record", "",
  "초과 판정이 개별 측정값이 아니라 이 값으로 이뤄진다. 고시가 따옴표로 정의어를 부여한 "
  "정식 용어다. 5분·시간·24시간으로 층이 나뉘는 자료 주기 자체는 컬럼이 아니라 값이므로 "
  "엔진의 코드값 세트로 둔다(원전에 주기 구분 컬럼명이 없다)",
  "제13조제1항 '행정자료 활용을 위해 \"24시간 평균치\" 생성', 제13조제3항. "
  "생성방법은 별표3이며 별표3은 확보 범위 밖이다",
  basis=TMS_REG + " 제13조제1항", review="verified")
d("MD-073", "일일 평균 농도", "Daily Average Concentration", "-", "", "numeric", "record", "",
  "대국민 공개 자료의 법정 항목. 폐수 업체가 실제로 마주하는 공개 데이터셋의 컬럼 이름이 "
  "이것이므로 사전에 없으면 공개 데이터 매핑이 바로 실패한다. MD-072 24시간 평균치와 "
  "같은 값인지 다른 값인지는 고시가 정의하지 않으므로 동일시하지 말 것",
  "제13조의2제1항 '수질오염물질 일일 평균 농도 및 일일 배출량'",
  basis=TMS_REG + " 제13조의2제1항", review="verified")
d("MD-074", "일일 배출량", "Daily Discharge Load", "-", "", "numeric", "record", "",
  "농도와 구분되는 부하량 축. WQ-078 폐수배출량(수량, ㎥/일)·WQ-075 유량(㎥/s)과 차원이 "
  "다르므로 세 개념의 경계를 지켜야 중복 등록을 피한다",
  "제13조의2제1항", basis=TMS_REG + " 제13조의2제1항", review="verified")
